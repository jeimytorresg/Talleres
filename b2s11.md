...
```scala
  val promedioRevenue = rows.map(_.revenue).sum / rows.length.toDouble
  val peliSobrePromedio = rows.filter(_.revenue > promedioRevenue)
  println(s"Películas con ingresos mayores al promedio: ${peliSobrePromedio.length}")

  val peliculasAdultos = rows.count(_.adult)
  println(s"Cantidad de películas para adultos: $peliculasAdultos")

  val votosPromedio = rows.map(_.vote_average).sum / rows.length
  val varianza = rows.map(movie => math.pow(movie.vote_average - votosPromedio, 2)).sum / rows.length
  println(f"Varianza de las calificaciones (vote_average): $varianza%.2f")
```
